...
class DeleteSocialMgmtTest extends TestCase
{
  ...
  protected string $deleteUrl = '/api/admin/social-mgmt/delete'; // POST endpoint
  ...
  public function test_delete_single_success()
  {
    ...
    $target = SocialMgmt::factory()->create();
  ...
  public function test_delete_multiple_success()
  {
    ...
    $target1 = SocialMgmt::factory()->create();
    $target2 = SocialMgmt::factory()->create();

    $response = $this->call('POST', $this->deleteUrl, ['ids' => [$target1->id, $target2->id]], $cookies);

    $response->assertStatus(CommonVal::HTTP_OK);

    $this->assertDatabaseHas('social_mgmt', ['id' => $target1->id, 'is_delete' => 1]);
    $this->assertDatabaseHas('social_mgmt', ['id' => $target2->id, 'is_delete' => 1]);
  }
