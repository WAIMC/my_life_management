...
class DeleteSettingLinkMgmtTest extends TestCase
{
  use DatabaseTransactions;

  protected string $deleteUrl = '/api/admin/setting-link-mgmt/delete'; // POST endpoint
  ...
  public function test_delete_single_success()
  {
    ...
    $target = SettingLinkMgmt::factory()->create();
    ...
    $this->assertDatabaseHas('setting_link_mgmt_hist', [
      'setting_link_mgmt_id' => $target->id,
  ...
  public function test_delete_multiple_success()
  {
    ...
    $target1 = SettingLinkMgmt::factory()->create();
    $target2 = SettingLinkMgmt::factory()->create();

    $response = $this->call('POST', $this->deleteUrl, ['ids' => [$target1->id, $target2->id]], $cookies);

    $response->assertStatus(CommonVal::HTTP_OK);

    $this->assertDatabaseHas('setting_link_mgmt', ['id' => $target1->id, 'is_delete' => 1]);
    $this->assertDatabaseHas('setting_link_mgmt', ['id' => $target2->id, 'is_delete' => 1]);
  }
