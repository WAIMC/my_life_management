...
class DeleteSkillMgmtTest extends TestCase
{
  use DatabaseTransactions;

  protected string $deleteUrl = '/api/admin/skill-mgmt/delete'; // POST endpoint
  ...
  public function test_delete_single_success()
  {
    ...
    $target = SkillMgmt::factory()->create();
  ...
  public function test_delete_multiple_success()
  {
    ...
    $target1 = SkillMgmt::factory()->create();
    $target2 = SkillMgmt::factory()->create();

    $response = $this->call('POST', $this->deleteUrl, ['ids' => [$target1->id, $target2->id]], $cookies);

    $response->assertStatus(CommonVal::HTTP_OK);

    $this->assertDatabaseHas('skill_mgmt', ['id' => $target1->id, 'is_delete' => 1]);
    $this->assertDatabaseHas('skill_mgmt', ['id' => $target2->id, 'is_delete' => 1]);
  }
