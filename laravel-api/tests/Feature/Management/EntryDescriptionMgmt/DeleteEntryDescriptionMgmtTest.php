...
class DeleteEntryDescriptionMgmtTest extends TestCase
{
  use DatabaseTransactions;

  protected string $deleteUrl = '/api/admin/entry-description-mgmt/delete'; // POST endpoint
  ...
  public function test_delete_single_success()
  {
    ...
    $this->assertDatabaseHas('entry_description_mgmt_hist', [
      'entry_description_mgmt_id' => $target->id,
  ...
  public function test_delete_multiple_success()
  {
    ...
    $target1 = EntryDescriptionMgmt::factory()->create();
    $target2 = EntryDescriptionMgmt::factory()->create();

    $response = $this->call('POST', $this->deleteUrl, ['ids' => [$target1->id, $target2->id]], $cookies);

    $response->assertStatus(CommonVal::HTTP_OK);

    $this->assertDatabaseHas('entry_description_mgmt', ['id' => $target1->id, 'is_delete' => 1]);
    $this->assertDatabaseHas('entry_description_mgmt', ['id' => $target2->id, 'is_delete' => 1]);
  }
