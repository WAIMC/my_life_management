...
class DeleteSkillDescriptionMgmtTest extends TestCase
{
  use DatabaseTransactions;

  protected string $deleteUrl = '/api/admin/skill-description-mgmt/delete'; // POST endpoint
  ...
  public function test_delete_single_success()
  {
    ...
    $target = SkillDescriptionMgmt::factory()->create();

    $response = $this->call('POST', $this->deleteUrl, ['ids' => [$target->id]], $cookies);

    $response->assertStatus(CommonVal::HTTP_OK);

    $this->assertDatabaseHas('skill_description_mgmt', [
      'id' => $target->id,
      'is_delete' => 1
    ]);

    $this->assertDatabaseHas('skill_description_mgmt_hist', [
      'skill_description_mgmt_id' => $target->id,
  ...
  public function test_delete_multiple_success()
  {
    ...
    $target1 = SkillDescriptionMgmt::factory()->create();
    $target2 = SkillDescriptionMgmt::factory()->create();

    $response = $this->call('POST', $this->deleteUrl, ['ids' => [$target1->id, $target2->id]], $cookies);

    $response->assertStatus(CommonVal::HTTP_OK);

    $this->assertDatabaseHas('skill_description_mgmt', ['id' => $target1->id, 'is_delete' => 1]);
    $this->assertDatabaseHas('skill_description_mgmt', ['id' => $target2->id, 'is_delete' => 1]);
  }
