...
class SkillIntegrationTest extends TestCase
{
  ...
  public function test_skill_catalog_building_flow()
  {
    $admin = AdminMst::factory()->create();

    $this->ensureRootAccess($admin, 'POST', $this->loginUrl);
    $this->ensureRootAccess($admin, 'POST', 'api/admin/category-mgmt/store');
    $this->ensureRootAccess($admin, 'POST', 'api/admin/skill-mgmt/store');
    $this->ensureRootAccess($admin, 'PUT', 'api/admin/category-skill-mgmt/update');
    $this->ensureRootAccess($admin, 'POST', 'api/admin/skill-description-mgmt/store');
    ...
    $skillResp = $this->call('POST', 'api/admin/skill-mgmt/store', $skillPayload, $cookies);
    $skillResp->assertStatus(200);
    $skillId = $skillResp->json('data');

    // 4. Link Skill to Category (PUT)
    $linkPayload = [
      'insert' => [
        [
          'category_mgmt_id' => $catId,
          'skill_mgmt_id' => $skillId
        ]
      ]
    ];
    $linkResp = $this->call('PUT', 'api/admin/category-skill-mgmt/update', $linkPayload, $cookies);
    ...
    $descResp = $this->call('POST', 'api/admin/skill-description-mgmt/store', $descPayload, $cookies);
    $descResp->assertStatus(200);

    // 6. Verify Connection
    $this->assertDatabaseHas('category_skill_mgmt', ['category_mgmt_id' => $catId, 'skill_mgmt_id' => $skillId]);
  }
