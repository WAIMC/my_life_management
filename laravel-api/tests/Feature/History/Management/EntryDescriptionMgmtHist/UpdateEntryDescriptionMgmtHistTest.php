...
class UpdateEntryDescriptionMgmtHistTest extends TestCase
{
    use RefreshDatabase;

    private string $baseUrl = 'api/admin/entry-description-mgmt-hist/update';
    ...
    public function test_SKL_DSC_HST_UPD_001_unauthenticated()
    {
        $response = $this->getJson($this->baseUrl);
        $response->assertStatus(401);
    }

    public function test_SKL_DSC_HST_UPD_002_success_list()
    {
        $admin = AdminMst::factory()->create();
        $cookies = $this->getAuthCookies($admin);

        $desc = EntryDescriptionMgmt::factory()->create();
        EntryDescriptionMgmtHist::create([
    ...
    public function test_SKL_DSC_HST_UPD_003_filter_by_desc()
    {
        $admin = AdminMst::factory()->create();
        $cookies = $this->getAuthCookies($admin);

        $desc1 = EntryDescriptionMgmt::factory()->create();
        $desc2 = EntryDescriptionMgmt::factory()->create();
        ...
        EntryDescriptionMgmtHist::create([
            ...
            'entry_mgmt_id' => $desc1->entry_mgmt_id,
            'action' => 1,
            'author_id' => $admin->id,
            'created_at' => now(),
        ]);

        $response = $this->call('PUT', $this->baseUrl, ['entry_description_mgmt_id' => $desc1->id], $cookies);
        $response->assertStatus(200);
