...
class EntryDescriptionMgmtHistRepository extends BaseRepository implements EntryDescriptionMgmtHistInterface
{
  ...
  public function list(array $payload): LengthAwarePaginator
  {
    $query = $this->model->query()
      ->select(['id', 'entry_description_mgmt_id', 'parent_id', 'title', 'summary', 'article', 'status', 'is_display', 'rank_order', 'entry_mgmt_id', 'action', 'author_id'])
      ->with(['entryDescriptionMgmt:id,title', 'author:id,user_name']);
    $this->applyFilters($query, $payload, ['entry_description_mgmt_id', 'parent_id', 'status', 'is_display', 'rank_order', 'entry_mgmt_id', 'action', 'author_id'], ['title', 'summary', 'article']);
