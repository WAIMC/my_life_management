...
export const skillDescriptionService = {
  async list(params: ListQueryParams = {}) {
    return apiClient.get<PaginatedResponse<SkillDescriptionMgmt>>(ENDPOINTS.MANAGEMENT.SKILL_DESCRIPTION, params);
  },
  ...
  async create(data: Omit<SkillDescriptionMgmt, 'id' | 'updated_at' | 'created_at'>) {
    return apiClient.post<number>(ENDPOINTS.MANAGEMENT.SKILL_DESCRIPTION, data);
  },
  async update(id: number, data: Partial<SkillDescriptionMgmt>) {
    return apiClient.put<number>(`${ENDPOINTS.MANAGEMENT.SKILL_DESCRIPTION}/${id}`, { id, ...data });
  },
  async delete(ids: number[]) {
    await apiClient.delete(ENDPOINTS.MANAGEMENT.SKILL_DESCRIPTION, { ids });
  },
};
