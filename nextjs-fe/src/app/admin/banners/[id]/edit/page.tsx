...
export default function EditBannerPage() {
  ...
  const { update, loading } = useCrud<BannerMgmt>(ENDPOINTS.MANAGEMENT.BANNER);
      ...
      try {
        ...
        if (banner) {
          form.reset({
            ...
            rank_order: banner.rank_order,
            status: banner.status,
            is_active: banner.is_active,
          });
        } else {
          notification.error('Banner not found');
          router.push('/admin/banners');
        }
      } catch (error) {
        notification.error('Failed to load banner');
        router.push('/admin/banners');
      }
    };

    loadBanner();
  }, [bannerId, form, router]);

  const onSubmit = async (data: BannerFormData) => {
    await update(bannerId, data);
    router.push('/admin/banners');
  };
