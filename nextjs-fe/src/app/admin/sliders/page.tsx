  ...
  return (
    <AdminLayout>
      <PageHeader
        ...
        action={<Can module="slider" action="create"><Button onClick={() => router.push('/admin/sliders/create')}>Create Slider</Button></Can>}
      />

      <div className="mt-6 space-y-4"><div className="flex gap-2"><AdvancedSearch fields={searchFields} onSearch={handleAdvancedSearch} /><SavedFilters currentFilters={filters} onLoad={(f) => { setFilters(f); setPage(1); }} filterKey="slider-filters" /><Can module="slider" action="export"><ImportExport data={data} onImport={handleImport} filename="sliders-export" /></Can></div><FilterPanel
          filters={filters}
        ...
        <Can module="slider" action="delete"><BulkActions selectedIds={selectedIds} onClearSelection={() => setSelectedIds([])} actions={bulkActions} isLoading={loading} /></Can>
