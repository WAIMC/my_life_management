...
export default function EditSliderPage() {
  ...
  useEffect(() => {
    const loadSlider = async () => {
      try {
        const slider = await sliderService.getById(sliderId);
        if (slider) {
          form.reset({
            title: slider.title,
            ...
            rank_order: slider.rank_order,
            status: slider.status,
            is_active: slider.is_active,
          });
        } else {
          notification.error('Slider not found');
          router.push('/admin/sliders');
        }
      } catch (error) {
        notification.error('Failed to load slider');
        router.push('/admin/sliders');
      }
    };

    loadSlider();
  }, [sliderId, form, router]);

  const onSubmit = async (data: SliderFormData) => {
    await update(sliderId, data);
    router.push('/admin/sliders');
  };
