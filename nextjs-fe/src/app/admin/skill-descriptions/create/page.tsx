...
export default function CreateSkillDescriptionPage() {
  const router = useRouter();
  const { create, loading } = useCrud<SkillDescriptionMgmt>(ENDPOINTS.MANAGEMENT.SKILL_DESCRIPTION);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end gap-4 pt-4 border-t">
            ...
            <Button type="submit" disabled={loading || skillsLoading}>
              {loading ? 'Creating...' : 'Create Skill Description'}
            </Button>
          </div>
        </form>
      </div>
