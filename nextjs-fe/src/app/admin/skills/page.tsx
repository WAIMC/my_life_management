  ...
  return (
    <AdminLayout>
      <PageHeader
        ...
        action={<Can module="skill" action="create"><Button onClick={() => router.push('/admin/skills/create')}>Create Skill</Button></Can>}
      />

      <div className="mt-6 space-y-4"><div className="flex gap-2"><AdvancedSearch fields={searchFields} onSearch={handleAdvancedSearch} /><SavedFilters currentFilters={filters} onLoad={(f) => { setFilters(f); setPage(1); }} filterKey="skill-filters" /><Can module="skill" action="export"><ImportExport data={data} onImport={handleImport} filename="skills-export" /></Can></div><FilterPanel
          filters={filters}
        ...
        <Can module="skill" action="delete"><BulkActions selectedIds={selectedIds} onClearSelection={() => setSelectedIds([])} actions={bulkActions} isLoading={loading} /></Can>
